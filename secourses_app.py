...
save_metadata =False ,metadata_params_base :dict =None 
    ...
    try :
                ...
                if save_chunks and scene_output_dir and scene_name :
                    ...
                    if save_metadata and metadata_params_base :
                        try :
                            ...
                            # Define the directory for this scene's chunk progress metadata.
                            # It should be relative to the scene's own output directory.
                            if scene_output_dir and scene_name: # scene_output_dir is frames_output_subfolder from run_upscale
                                # Path: [frames_output_subfolder]/scenes/[scene_name]/chunk_progress_metadata/
                                ...
                                # Fallback: if somehow scene_output_dir or scene_name is not set,
                                # save to a temporary location to avoid crashing.
                            ...
                            temp_params_for_chunk_meta ['input_fps']=scene_fps
                            # Ensure the prompt used for this scene's chunks is included
                            # scene_prompt is from auto-caption if enabled for this scene, otherwise final_prompt is the original.
        ...
        if save_metadata and metadata_params_base and scene_output_dir :
            ...
            "processing_time_total":scene_duration 
            }
            ...
            params_dict =metadata_params_base ,
            status_info =final_scene_status_info ,
            logger =logger 
            )
...
progress =gr .Progress (track_tqdm =True )
    ...
    try :
        ...
        if enable_scene_split and scene_video_paths :
            ...
            for scene_idx ,scene_video_path in enumerate (scene_video_paths ):
                ...
                try :
                    ...
                    metadata_params_base =scene_metadata_base_params 
                    )
        ...
        if not enable_scene_split :
            ...
            current_overall_progress =initial_progress_silent_video 
    ...
    progress =gr .Progress (track_tqdm =True )
        ...
        if do_auto_caption_first_val and app_config .UTIL_COG_VLM_AVAILABLE :
            ...
            try :
                ...
                input_video_val ,
                actual_cogvlm_quant_for_captioning ,
        ...
        do_auto_caption_first_val and enable_scene_split_check_val and app_config .UTIL_COG_VLM_AVAILABLE ,
        cogvlm_quant_radio_val ,
